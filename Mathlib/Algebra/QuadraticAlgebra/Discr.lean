...
@[simp]
theorem re_algEquivDiscrZero_apply [Invertible (2 : R)] {a b : R} (z : QuadraticAlgebra R a b) :
    (algEquivDiscrZero a b z).re = z.re + ⅟2 * b * z.im := by
  simp [algEquivDiscrZero]
  ring

@[simp]
theorem im_algEquivDiscrZero_apply [Invertible (2 : R)] {a b : R} (z : QuadraticAlgebra R a b) :
    (algEquivDiscrZero a b z).im = ⅟2 * z.im := by
  simp [algEquivDiscrZero]
  ring

@[simp]
theorem re_algEquivDiscrZero_symm_apply [Invertible (2 : R)] {a b : R}
...
@[simp]
theorem im_algEquivDiscrZero_symm_apply [Invertible (2 : R)] {a b : R}
    (z : QuadraticAlgebra R (discr a b) 0) :
...
@[simp]
theorem algEquivDiscrZero_apply_add_smul [Invertible (2 : R)] {a b : R} (x y : R) :
    algEquivDiscrZero a b (x • 1 + y • ω) = (x + ⅟2 * b * y) • 1 + (⅟2 * y) • ω := by
  ext <;> simp <;> ring

@[simp]
theorem algEquivDiscrZero_symm_apply_add_smul [Invertible (2 : R)] {a b : R} (x y : R) :
